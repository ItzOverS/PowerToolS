...
public class AntiWorldDownLoader
    ...
    public AntiWorldDownLoader(boolean stats) {
        super("AntiWorldDownLoader", "1.0", "prevent players from download server's maps using mapDownloaders", "NONE", stats);
        if(stats) {
    ...
    public void onPluginMessageReceived(String channel, Player player, byte[] data) {
        if (channel.equals("WDL|INIT")) {
            ...
            if (!player.isOp()) {
                ...
                if (!config.getBoolean("AntiWorldDownLoader.continueOperators")) {
                    Bukkit.getScheduler().scheduleSyncDelayedTask(PowerTools.INSTANCE, () -> {
                        Bukkit.getPlayer(player.getName()).kickPlayer(PlInfo.KICK_PREFIX + ChatColor.RED + "I think you're using WorldDownLoader");
                        if(config.getBoolean(thisName + ".alert-on-discord"))
