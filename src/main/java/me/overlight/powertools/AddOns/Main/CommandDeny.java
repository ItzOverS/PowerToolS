...
public class CommandDeny
        extends AddOn
        implements Listener {
    public CommandDeny(boolean stats) {
        super("CommandDeny", "1.0", "deny commands", "NONE", stats);
    }
    ...
    public void onCommandExecute(PlayerCommandPreprocessEvent e){
        if(this.isEnabled()){
            ...
            if (config.getBoolean(this.getName() + ".denyPluginNameOnStart")) {
                if (command.contains(":")) {
                    if (config.getStringList(this.getName() + ".commands").contains(command.split(":")[1])) {
                        if (!e.getPlayer().isOp()) {
                            ...
                            if(!config.getBoolean(this.getName() + ".ignoreOps")) {
                                PowerTools.Alert(PowerTools.Target.STAFF, ChatColor.RED + e.getPlayer().getName() + ChatColor.GOLD + " faild to execute: " + e.getMessage().substring(1));
                                e.getPlayer().sendMessage(ChatColor.WHITE + "Unknown command. Type \"/help\" for help.");
                                e.setCancelled(true);
                                if(config.getBoolean(this.getName() + ".alert-on-discord"))
                    ...
                    if (config.getStringList(this.getName() + ".commands").contains(command)){
                        if (!e.getPlayer().isOp()) {
                            PowerTools.Alert(PowerTools.Target.STAFF, ChatColor.RED + e.getPlayer().getName() + ChatColor.GOLD + " faild to execute: " + e.getMessage().substring(1));
                            e.getPlayer().sendMessage(ChatColor.WHITE + "Unknown command. Type \"/help\" for help");
                            e.setCancelled(true);
